...
// TX 返回一个新的事务实例.目的是让此时处理的动作都在同一个事务中
func (store *datastore) TX(ctx context.Context, fn func(ctx context.Context) error) error {
	//把传入的ctx绑定到GORM的*gorm.DB实例上，Transaction是GORM的事务封装方法，内部会开始一个事务调用.Begin()
	//发生错误就Rollback()，正常就Commint()。
	return store.core.WithContext(ctx).Transaction(
		func(tx *gorm.DB) error {
			//把事务tx放进ctx，transaction{}为KEY，覆盖原来的ctx
